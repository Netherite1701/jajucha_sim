...
## Manual-compatible APIs verified in the simulator

| Feature | Manual behavior (target) | Status |
|---|---|---|
| `jchm.control.stop_motor()` | stop + center steering | IMPLEMENTED; delegates to `set_motor(0, 0, 0)` and has a unit test |
| `jchm.camera.get_image("left"\|"center"\|"right")` | BGR uint8 (h,w,3), JPEG | IMPLEMENTED; runtime smoke verifies all three frames |
| `jchm.camera.get_depth()` | grayscale uint8 (h,w), near=bright(255)/far=dark(0), gray8 | IMPLEMENTED; runtime smoke verifies the depth frame |
| `jchm.camera.show_image(...)` | local display; not a vehicle API | IMPLEMENTED (client-side, Python) |
| Bridge protocol | 4-byte big-endian length + UTF-8 JSON, localhost (default 127.0.0.1:8765) | IMPLEMENTED; bridge smoke verifies handshake and binary frames |
